...
public class CustomerBusinessLogic {
	...
	public boolean verifyRegisteringCustomer(Customer customer) {
		...
		try {
			...
			// Customer
			// existingCustomer=customerRepository.findByCustomerEmail(customer.getCustomerEmail()).orElseThrow(()->new
			// InputException("customer already exists"));
			Optional<Customer> customerOptional = customerRepository.findByCustomerEmail(customer.getCustomerEmail());
			Customer savedUser = customerOptional
					.orElseThrow(() -> new InputException("customer is not there in the table"));
